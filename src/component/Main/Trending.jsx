...
function Trending() {
  ...
  // currentPage is 0-based (matches ReactPaginate)
  const [currentPage, setCurrentPage] = useState(0);

  // pageCount from redux (ensure it's a number)
  const pageCount = Number(MovieListArray?.PaginationTrending) || 0;

  const handlePageClick = (event) => {
    const selected0Based = event.selected; // 0-based
    setCurrentPage(selected0Based);
    // dispatch API call with 1-based page
    dispatch(
      TrendingTodayFetch({
  ...
  if (!MovieListArray?.trending || MovieListArray?.trending?.length === 0)
    return null;
